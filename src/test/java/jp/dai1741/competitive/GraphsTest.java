...
import jp.dai1741.competitive.Graphs.FlowAdjGraph;
import jp.dai1741.competitive.Graphs.EdgeGraph;
import jp.dai1741.competitive.Graphs.AdjGraph;
import jp.dai1741.competitive.Graphs.MatrixGraph;
...
public class GraphsTest {
    ...
    static class AllGraph {
        ...
        AdjGraph adjGraph;
        EdgeGraph edgeGraph;
        FlowAdjGraph flowAdjGraph;
        int n;

        public AllGraph(int n) {
            this.n = n;
            matrixGraph = new MatrixGraph(n);
            adjGraph = new AdjGraph(n);
            edgeGraph = new EdgeGraph(n);
            flowAdjGraph = new FlowAdjGraph(n);
        }

        void addArc(Edge e) {
            matrixGraph.addArc(e);
            adjGraph.addArc(e);
            edgeGraph.addArc(e);
            flowAdjGraph.addArc(e);
        }

        void addEdge(Edge e) {
            matrixGraph.addEdge(e);
            adjGraph.addEdge(e);
            edgeGraph.addEdge(e);
            flowAdjGraph.addEdge(e);
        }
    }
    ...
    public void testGraphStructure() {
        ...
        assertEquals(2, g.adjGraph.edges[0].size());
        assertEquals(6, g.edgeGraph.edges.size());
        ...
        assertEquals(5, g.adjGraph.edges[0].get(1).to);
        assertEquals(4, g.adjGraph.edges[3].get(0).to);
        assertEquals(8, g.adjGraph.edges[8].get(0).from);
        assertEquals(1, g.adjGraph.edges[1].get(0).cost);

        assertEquals(1, g.edgeGraph.edges.get(2).from);
        assertEquals(2, g.edgeGraph.edges.get(2).to);
        assertEquals(1, g.edgeGraph.edges.get(4).cost);
    }
    ...
    public void testDijkstra() {
        ...
        dijkstraWithPath(g.adjGraph, 3, prev);
        assertArrayEquals(expectedPrev, prev);
    ...
    public void testBellmanFord() {
        ...
        assertArrayEquals(distFrom0, bellmanFord(g.adjGraph, 0));
        int[] distFrom3 = { -2, 1, 2, 0, INF, 0, 3 };
        assertArrayEquals(distFrom3, bellmanFord(g.matrixGraph, 3));
        assertArrayEquals(distFrom3, bellmanFord(g.adjGraph, 3));
        // implement prev?
        ...
        assertArrayEquals(distFrom4, bellmanFord(loopG.adjGraph, 4));
    ...
    public void testWarshallFloyd() {
        ...
        assertArrayEquals(dists, warshallFloyd(g.adjGraph));
        ...
        assertTrue(warshallFloyd(loopG.adjGraph)[2][2] < 0);
    }
    ...
    public void testKruskal() {
        ...
        assertEquals(5, kruskal(g.edgeGraph));

        AllGraph loopG = makeGraph("6 "
                + "0 1 3  0 4 1  1 2 1  2 3 1  3 1 -3  3 4 3  4 5 -1", true, false, true);
        assertEquals(1, kruskal(loopG.edgeGraph));

        assertEquals(0, kruskal(noedges.edgeGraph)); // ??
    }
    ...
    public void testEdmondsKarp() {
        ...
        assertEquals(5, edmondsKarp(g.flowAdjGraph, 0, 7));
        g.flowAdjGraph.resetFlow();
        assertEquals(0, edmondsKarp(g.flowAdjGraph, 0, 6));
        ...
        assertEquals(59, edmondsKarp(g.flowAdjGraph, 0, 6));


        assertEquals(0, edmondsKarp(noedges.flowAdjGraph, 0, 4));

        AllGraph multiples = makeGraph("2 " + "0 1 10  0 1 10 0 1 20  ", false, true);
        assertEquals(40, edmondsKarp(multiples.flowAdjGraph, 0, 1));
        ...
        assertEquals(4, edmondsKarp(bipartite.flowAdjGraph, 0, 11));
        AllGraph big = makeGraph(
                ...
                false, true);
        assertEquals(532, edmondsKarp(big.flowAdjGraph, 1, 99));
    }
    ...
    public void testBipartiteMatching() {
        AllGraph g = makeGraph("12 "
                + "1 7 1  2 6 1  2 7 1  2 9 1  3 8 1  4 7 1  5 8 1  5 9 1  5 10 1",
                false, true, true);
        assertEquals(4, bipartiteMatching(g.adjGraph));
        g = new AllGraph(1000);
        for (int i = 0; i < 500; i++) {
            g.addEdge(new Edge(i, i * i % 500 + 500, 0, 1));
        }
        assertEquals(106, bipartiteMatching(g.adjGraph));
    }
    ...
    public void testMinCostFlowWithBellmanFord() {
        ...
        assertEquals(11, edmondsKarp(g.flowAdjGraph, 0, 7));
        g.flowAdjGraph.resetFlow();
        assertEquals(94, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 7, 9));
        g.flowAdjGraph.resetFlow();
        assertEquals(-1, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 6, 6));
        g.flowAdjGraph.resetFlow();

        g = reversingCostFlowGraph;

        assertEquals(12, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 3, 4));
        g.flowAdjGraph.resetFlow();
        assertEquals(15, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 3, 5));
        g.flowAdjGraph.resetFlow();
        assertEquals(20, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 3, 6));
        g.flowAdjGraph.resetFlow();
        assertEquals(40, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 3, 10));
        g.flowAdjGraph.resetFlow();
        assertEquals(-1, minCostFlowWithBellmanFord(g.flowAdjGraph, 0, 3, 11));
        g.flowAdjGraph.resetFlow();

        assertEquals(-1, minCostFlowWithBellmanFord(noedges.flowAdjGraph, 0, 4, 1));
        assertEquals(0, minCostFlowWithBellmanFord(noedges.flowAdjGraph, 0, 4, 0));

        AllGraph multiples = makeGraph("2 " + "0 1 1 10  0 1 2 10 0 1 3 20  ", true, true);
        assertEquals(60, minCostFlowWithBellmanFord(multiples.flowAdjGraph, 0, 1, 30));
    }
    ...
    public void testMinCostFlowWithPrimalDual() {
        ...
        assertEquals(94, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 7, 9));
        g.flowAdjGraph.resetFlow();
        assertEquals(-1, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 6, 6));
        g.flowAdjGraph.resetFlow();

        g = reversingCostFlowGraph;

        assertEquals(12, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 3, 4));
        g.flowAdjGraph.resetFlow();
        assertEquals(15, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 3, 5));
        g.flowAdjGraph.resetFlow();
        assertEquals(20, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 3, 6));
        g.flowAdjGraph.resetFlow();
        assertEquals(40, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 3, 10));
        g.flowAdjGraph.resetFlow();
        assertEquals(-1, minCostFlowWithPrimalDual(g.flowAdjGraph, 0, 3, 11));
        g.flowAdjGraph.resetFlow();

        assertEquals(-1, minCostFlowWithPrimalDual(noedges.flowAdjGraph, 0, 4, 1));
        assertEquals(0, minCostFlowWithPrimalDual(noedges.flowAdjGraph, 0, 4, 0));

        AllGraph multiples = makeGraph("2 " + "0 1 1 10  0 1 2 10 0 1 3 20  ", true, true);
        assertEquals(60, minCostFlowWithPrimalDual(multiples.flowAdjGraph, 0, 1, 30));
    }
    ...
    public void testDecomposeIntoStronglyConnectedComponents() {
        ...
        assertEquals(2, decomposeIntoStronglyConnectedComponents(
                standardWaightedGraph.adjGraph, components));
        assertArrayEquals(new int[] { 1, 1, 1, 1, 1, 1, 0, 1 }, components);

        StronglyConnectedComponents scc = new StronglyConnectedComponents(
                standardWaightedGraph.adjGraph);
        ...
        assertEquals(4, decomposeIntoStronglyConnectedComponents(straightTree.adjGraph,
                components));
        assertArrayEquals(new int[] { 1, 1, 1, 0, 2, 3 }, components);

        scc = new StronglyConnectedComponents(straightTree.adjGraph);
        ...
        assertEquals(3, decomposeIntoStronglyConnectedComponents(smallForest.adjGraph,
                components));
        assertTrue(components[0] < components[1]);
        assertTrue(components[0] != components[2]);
        assertTrue(components[1] != components[2]);

        scc = new StronglyConnectedComponents(smallForest.adjGraph);
    ...
    public void testViterbi() {
        ...
        assertEquals(4, viterbi(aDag.adjGraph, 3, 2, components));
        assertEquals(8, viterbi(aDag.adjGraph, 3, 0, components));
        assertEquals(INF, viterbi(aDag.adjGraph, 3, 5, components));
        assertEquals(15, viterbi(aDag.adjGraph, 5, 0, components));
        assertEquals(8, viterbi(aDag.adjGraph, 2, 0, components));
        assertEquals(14, viterbi(aDag.adjGraph, 5, 4, components));
        ...
        assertEquals(INF, viterbi(aDisConnectedDag.adjGraph, 6, 7, components));
        assertEquals(INF, viterbi(aDisConnectedDag.adjGraph, 4, 1, components));
        assertEquals(INF, viterbi(aDisConnectedDag.adjGraph, 1, 5, components));
        assertEquals(58, viterbi(aDisConnectedDag.adjGraph, 3, 7, components));
        assertEquals(69, viterbi(aDisConnectedDag.adjGraph, 0, 2, components));
        assertEquals(INF, viterbi(aDisConnectedDag.adjGraph, 2, 0, components));
    }
    ...
    public void testBigRandomGraph() {
        ...
        assertEquals(730, dijkstra(g.adjGraph, 0, 500));
        assertEquals(730, dijkstra(g.matrixGraph, 0, 500));
        assertEquals(888, dijkstra(g.matrixGraph, 777, 444));
        assertEquals(888, bellmanFord(g.adjGraph, 777)[444]);
        // assertEquals(888, bellmanFord(g.matrixGraph, 777)[444]); // TLE
        assertEquals(997, dijkstra(g.adjGraph, 256, 8));
        assertEquals(56947, kruskal(g.edgeGraph));
        assertEquals(4166, edmondsKarp(g.flowAdjGraph, 0, 1));

        AdjGraph lg = new AdjGraph(100000);
        FlowAdjGraph flg = new FlowAdjGraph(100000);
